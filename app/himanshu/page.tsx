...
export default function HimanshuDashboard() {
  ...
  const { data: summariesData, error: summariesError, mutate: mutateSummaries } = useSWR(
    `/api/summaries?interval=${selectedInterval}&user=himanshu`,
    fetcher
  );
  const { data: statsData, error: statsError } = useSWR('/api/stats?user=himanshu', fetcher);
      ...
      {summariesData && summaries.length === 0 && (
        <div className="text-center py-12">
          ...
          <p className="text-sm text-gray-400">
            Click &quot;Sync with WakaTime&quot; to fetch your activity data.
          </p>
        </div>
      )}
